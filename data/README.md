...
*Note on Scientific Context:* In early 2025, Hugging Face archived the Open LLM Leaderboard V2 and halted the publication of raw instance-level prediction traces (JSONL) to combat benchmark dataset contamination. While newer iterations (like V3) rely on "hidden" evaluation sets to prevent cheating, this opacity prevents the instance-level scrutiny required for statistical bootstrapping. Thus, we explicitly utilize the archived V2 dataset as it represents the last fully transparent snapshot of top open-weights models at peak benchmark saturation. Even if specific models benefited from contamination, this only exacerbates score clustering at the top, making V2 the perfect real-world case study. HERA demonstrates that such tight point-estimate margins are statistically meaningless and overlapping, proving that robust statistical ranking is a superior alternative to sacrificing open science for hidden data. (For Hugging Face's official acknowledgment of how data contamination forced continuous leaderboard overhauls, see their [Open LLM Leaderboard v2 Launch Blog Post](https://huggingface.co/spaces/open-llm-leaderboard/blog)).
