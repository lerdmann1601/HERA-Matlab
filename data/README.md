...
*Note on Scientific Context:* The Hugging Face Open LLM Leaderboard V2 was officially archived in early 2025, and the publication of raw instance-level prediction traces (JSONL) was halted to combat severe benchmark dataset contamination (where models implicitly overfit to static test sets). While this shift towards "hidden" evaluation sets aims to prevent cheating, it fundamentally harms open science by making independent reproducibility and instance-level scrutiny impossible. Alarmingly, even within these new opaque evaluation paradigms, models are still ranked using fragile, single point estimates. This context perfectly underscores the core motivation behind HERA: by utilizing this historical snapshot of top open-weights models—one of the last fully transparent datasets—HERA demonstrates how statistical bootstrapping could have revealed the true, overlapping rank stabilities without the community having to sacrifice open science and raw data transparency. Furthermore, even if specific models benefited from dataset contamination, this only exacerbates the clustering of scores at the top of the leaderboard (benchmark saturation). This makes it the perfect real-world case study for HERA, which proves that the resulting tight point-estimate margins are statistically meaningless and the rankings are heavily overlapping. (For Hugging Face's official acknowledgment of how benchmark saturation and data contamination forced them to continuously overhaul their leaderboards, see their [Open LLM Leaderboard v2 Launch Blog Post](https://huggingface.co/spaces/open-llm-leaderboard/blog)).
