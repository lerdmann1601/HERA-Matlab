...
> [!NOTE]
> The example results and interpretations shown below serve as a didactic walkthrough to demonstrate HERA's decision-making capabilities (based on HERA v1.0.0). Please be aware that the current version of the toolbox (v1.2.1) includes algorithmic refinements and performance optimizations. Consequently, running the examples with the latest code may yield slight variations in exact confidence intervals or tie-breaking nuances compared to these static screenshots. These differences reflect the improved precision of the current version, while the fundamental logic of the workflow remains consistent.
